...
| Model | Size | Notes |
|-------|------|-------|
| `apple:system` (Apple Intelligence) | Built-in | Optional on macOS 26+. The OS manages the on-device SystemLanguageModel. |
...
On macOS 26 or later, users can explicitly select Apple's built-in `SystemLanguageModel` when Apple Intelligence is available.
Steno does not silently change an existing model selection or fall back to another provider if Apple becomes unavailable.

On Apple Silicon, the three Gemma 4 models are automatically pulled as MLX/NVFP4 builds instead of the base GGUF, providing faster inference at the larger download sizes shown above.
